...
const CustomSlider: React.FC<SliderProps> = ({ minValue, maxValue, value, setValue, onChangeEnd }) => {
  ...
  const updateSliderByValue = (val: number) => {
    const clampedVal = Math.min(maxValue, Math.max(minValue, val));
    const offset = -((clampedVal - minValue) / step) * stepWidth + (containerRef.current!.offsetWidth / 2);
    setOffsetX(Math.round(offset / stepWidth) * stepWidth);
    setValue(clampedVal);
  };
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      const clampedVal = Math.min(maxValue, Math.max(minValue, sliderVal));
      setValue(clampedVal);
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      updateSliderByValue(Number(inputValue));
    }
  };
